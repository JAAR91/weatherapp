...
let showResults = (weatherBlock) => {
    ...
    <div class="row m-0 p-0">
        <div class="col-6 p-4 d-flex flex-row justify-content-center">
            ...
            <p class="temp m-0 p-0">${parseInt(weatherBlock.main.temp) - 273}°</p>
        </div>
        <div class="col-6 p-3">
            <div class="border-start p-3">
            <p class="text-white fs-3">Feels like: ${parseInt(weatherBlock.main.feels_like - 273)}°</p>
            <p class="text-white fs-3">Min/Max Temp: ${parseInt(weatherBlock.main.temp_min - 273)}°/${parseInt(weatherBlock.main.temp_max - 273)}°</p>
            <p class="text-white fs-3">Humidity: ${parseInt(weatherBlock.main.humidity)}%</p>
            <p class="text-white fs-3">Pressure: ${parseInt(weatherBlock.main.pressure)} Pas</p>
            </div>
        </div>
    </div>`;
...
let getWeather = async (city) => {
    try {
        let apiUrl = `http://api.openweathermap.org/data/2.5/weather?q=${city}&APPID=48cadb81651cd163e32a09db006ac294`
